...
@Component({
  ...
  animations: [
      trigger('fadeInOut', [
        ...
        transition(':enter', [animate('100ms ease-in')]), // Animation for entering
        transition(':leave', [animate('100ms ease-out')]) // Animation for leaving
      ]),
      trigger('slideIn', [
        state('void', style({ transform: 'translateX(-100%)', opacity: 0 })),
        transition(':enter', [animate('100ms ease-in')]), // Slide in from left
      ])
      ,
      trigger('slideInFromRight', [
        state('void', style({ transform: 'translateX(100%)', opacity: 0 })), // Start off-screen to the right
        transition(':enter', [
          animate('100ms ease-out', style({ transform: 'translateX(0)', opacity: 1 })) // Slide in to the screen
        ]),
        transition(':leave', [
          animate('100ms ease-in', style({ transform: 'translateX(100%)', opacity: 0 })) // Slide out back to the right
        ])
      ])
    ]
})
